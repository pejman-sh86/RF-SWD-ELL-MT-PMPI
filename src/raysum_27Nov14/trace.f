...
!JD        do isamp=1,nsamp
!JD          do itr=1,ntr
!JD            Tr_cart(1,isamp,itr)=0.
!JD            Tr_cart(2,isamp,itr)=0.
!JD            Tr_cart(3,isamp,itr)=0.
!JD          end do
!JD        end do
         Tr_cart=0.
...
c              write (*,*) 'Phase ',iph,' cropped out of trace ',itr
     ...
     &                        tt_s/dt,curamp,nsamp) 
                ...
                call putdelta(Tr_cart(1,1,itr),gwidth/(2.*dt),
     &                        tt_s/dt,curamp,nsamp) 
              endif
            end if
          end do
...
c Insert a delta pulse into a trace to get GF
      subroutine putdelta(traces,sigma,mu,amp,nsamp)
      
        implicit none
        include 'params.h'
        
        real traces(3,maxsamp),sigma,mu,amp(3)
        integer nsamp
        integer isamp
...
c        write (*,*) 'putgauss:',sigma,mu,amp
        ...
        if ((isamp .gt. 0) .and. (isamp .le. nsamp)) then
          traces(1,isamp)=traces(1,isamp)+amp(1)
          traces(2,isamp)=traces(2,isamp)+amp(2)
          traces(3,isamp)=traces(3,isamp)+amp(3)
        end if
...
c        write (*,*) 'sigma, mu, amp:',sigma,mu,amp
c        write (*,*) 'putgauss:',sigma,mu,amp
        do isamp=int(mu-5*sigma),int(mu+5*sigma)
          if ((isamp .gt. 0) .and. (isamp .le. nsamp)) then
c            write (*,*) 'gauss:',gauss(isamp)
