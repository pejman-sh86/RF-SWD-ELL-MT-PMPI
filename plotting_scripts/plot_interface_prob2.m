...
Pmax = [4.5 4. .6 .08]; %real
font_size = 30;
nx=Npanel; ny=2; 
% nx=1; ny=ength(deplim); Npanel=ny;
xim = 0.05; yim = 0.01;
% xim = 0.01; yim = 0.05;
...
% spw = spw*9./10.;
% for ipanel=1:Npanel
%     loc(1,ipanel) = loc(1,ipanel) - (ipanel-1)*(spw2-spw);
% end
% XTick = [0:5:10];
...
%         plot([dep_true(ipanel), dep_true(ipanel)],[ylim(1), ylim(2)],'--y','LineWidth',.5)
        ...
        if (ipanel==1); legend('MT','SWD-MT','RF-SWD-MT','True depth','FontSize',16); end
    end
