...
% 
% 
% usage: kolmogorov_dexp(data)
% 
% example usage: kolmogorov_dexp(data)
%
% Remark: 
...
% 
%
function[pass,value,bereik2,opp,dplot]=kolmogorov_depx(data,k);
%
%
%
[a b]=size(data);
...
p=minimaal;
for i=1:1:1000
   temp2(i,1)= (1/(s*sqrt(2))) * exp(-abs(p-gem)/s);
...
%hold on;
%plot(bereik2,opp,'r');
%plot(bereik2,dplot,'b');
%plot(gauss_x,gauss_y,'-k');
%yas1=blauw;
%yas2=rood;
%xas=[xas1 xas2];
%plot(bereik2(xas),[yas1 yas2],'g');
%legend('DEXP','exp dist',2);
%legend('boxoff');
%axis([minimaal maximaal 0 1.3]);
