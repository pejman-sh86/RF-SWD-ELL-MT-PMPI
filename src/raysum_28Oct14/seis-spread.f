...
c Generate phase list
        ...
        numph=0
        if ((mults .le. 2)) then  !! P to S conversion
          call ph_direct(phaselist,nseg,numph,nlay,iphase)
        end if
        if (mults .eq. 1) then !! Just Moho
          j=nlay-1
          call ph_fsmults(phaselist,nseg,numph,nlay,j,iphase)
        end if
        if (mults .eq. 2) then !! For all conversions (very expensive)
          do j=1,nlay-1
            call ph_fsmults(phaselist,nseg,numph,nlay,j,iphase)
          end do
        end if
        if (mults .eq. 3) then !! Just conversions visible in 
                               !! PS, PSPS, PPSS, PSSS
          j=nlay-1
          call ph_direct_rf(phaselist,nseg,numph,nlay,j,iphase)
          call ph_rfmults(phaselist,nseg,numph,nlay,j,iphase)
        end if
        if (mults .eq. 4) then !! All conversions
                               !!
          do j=1,nlay-1
            call ph_direct_rf(phaselist,nseg,numph,nlay,j,iphase)
          end do
