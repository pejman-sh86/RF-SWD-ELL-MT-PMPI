...
c New version with more efficient algorithm
     ...
     &           phaselist,ntr,nseg,numph,nlay,amp_in,errflag,ierr_neg)
...
c Interface variables:
        ...
        logical isoflag(maxlay),bailout,errflag
...
c            write (*,*) 'iph=',iph,' fchg=',fchg
     ...
     &                  nlay,amp_in,fchg,bailout,errflag,ierr_neg)
          end do
        end do 
...
c --------------------------------------------------------
     ...
     & baz,slow,phase,nseg,nlay,amp_in,fchg,bailout,errflag,ierr_neg)
...
c       Add post-interface segment to travel time
          ...
          if (thick(lay2) .lt. 0.) then
            ierr_neg = 1
            write (*,*) 'ERROR: negative thickness!',thick(lay2)
          end if
