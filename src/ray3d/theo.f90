subroutine theo(n, fbeta,rho2, h2, vps, qa, qb, fs, din, a0, c0, t0,nd, rx )
!*****************************************
...
!     **  h2(m) = thickness of the m-th layer
...
!
!
real w(nb2), wa(nb2), fai(nb2), gau(nb2),data(2*nb), &
     fbeta(nlmx), h2(nlmx), vps(nlmx), rho2(nlmx), &
     qa(nlmx), qb(nlmx), rs(nlmx),rx(nb)
!
complex up(nb2), wp(nb2), usv(nb2), wsv(nb2), vsh(nb2),&
        cc(nb),valpha(nlmx), vbeta(nlmx),yi, cs
!
!     *** parameters ***
yi=(0.,1.)
pi=3.141592653
...
!     **  computation for the case of incident p waves  **
      rin=pi*din/180.
      call qlayer(1, rin, n, h2, valpha, vbeta, rho2, qa, qb, rs,&
                  w, up, wp, usv, wsv, vsh )
!      write(*,*)'teo3'
