...
INTEGER(KIND=IB) :: u0,u1,u2,u3,u4,u5
...
IF(I_RV  >= 1)THEN
  OPEN(UNIT=u0,FILE=repfileR,FORM='formatted',STATUS='REPLACE', &
  ACTION='WRITE',RECL=1024)
ELSE
  OPEN(UNIT=u0,FILE=repfileRF,FORM='formatted',STATUS='REPLACE', &
  ACTION='WRITE',RECL=1024)
ENDIF
OPEN(UNIT=u1,FILE=repfileV,FORM='formatted',STATUS='REPLACE', &
ACTION='WRITE',RECL=1024)
OPEN(UNIT=u2,FILE=repfileS,FORM='formatted',STATUS='REPLACE', &
ACTION='WRITE',RECL=1024)
OPEN(UNIT=u3,FILE=repfileSWD,FORM='formatted',STATUS='REPLACE', &
ACTION='WRITE',RECL=1024)
OPEN(UNIT=u4,FILE=repfileSWDar,FORM='formatted',STATUS='REPLACE', &
ACTION='WRITE',RECL=1024)
IF(I_T   == 1)THEN
  OPEN(UNIT=u5,FILE=repfileT,FORM='formatted',STATUS='REPLACE', &
  ACTION='WRITE',RECL=1024)
ENDIF
...
DO ismp = 1,NSMP2-1
  ...
  CALL SAVEREPLICA(obj,u0,u1,u2,u3,u4,u5)
...
SUBROUTINE SAVEREPLICA(obj,u0,u1,u2,u3,u4,u5)
!=======================================================================
USE RJMCMC_COM
IMPLICIT NONE

INTEGER(KIND=IB) :: i,u0,u1,u2,u3,u4,u5
TYPE (objstruc)  :: obj      ! Best object

IF(I_RV >= 0)THEN
  DO i = 1,NRF1
    WRITE(u0,208) obj%DpredR(i,:)
  ENDDO
  DO i = 1,NRF1
    WRITE(u1,208) obj%DpredV(i,:)
  ENDDO
  DO i = 1,NRF1
    WRITE(u2,208) obj%S(i,:)
  ENDDO
  IF(I_T == 1)THEN
    DO i = 1,NRF1
      WRITE(u5,208) obj%DpredT(i,:)
    ENDDO
  ENDIF
ELSE
  DO i = 1,NRF1
...
IF(I_SWD == 1)THEN
  DO i = 1,NMODE
    WRITE(u3,208) obj%DpredSWD(i,:)
  ENDDO
  DO i = 1,NRF1
    WRITE(u4,208) obj%DarSWD(i,:)
  ENDDO
ENDIF 
