...
for iping = 1:5;
    c_med_new(1:idx(iping),iping) = min(min(squeeze(meact(:,3,:))))-10;
    c_med_new([1:NZ]+idx(iping),iping) = squeeze(meact(iping,3,:));
    c_med_new(NZ+idx(iping):end,iping) = min(min(squeeze(meact(:,3,:))))-10;

    r_med_new(1:idx(iping),iping) = min(min(squeeze(meart(:,3,:))))-.1;
    r_med_new([1:NZ]+idx(iping),iping) = meart(iping,3,:);
    r_med_new(NZ+idx(iping):end,iping) = min(min(meart(:,3,:)))-.1;

    a_med_new(1:idx(iping),iping) = min(min(squeeze(meaat(:,3,:))))-.1;
    a_med_new([1:NZ]+idx(iping),iping) = squeeze(meaat(iping,3,:));
    a_med_new(NZ+idx(iping):end,iping) = min(min(squeeze(meaat(:,3,:))))-.1;
