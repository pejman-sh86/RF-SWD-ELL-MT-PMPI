function [xVec,yVec,BW] = focalmech(fm, centerX, centerY, diam, iplot, varargin)

%close all; clear;
%fm = [-2.6133    2.1068    0.5065    0.0000    0.0000    0.9084];
%fm = [-1.0006    1.0011   -0.0005    1.0005   -1.0015   -1.0005];
%fm1 = [1.0006    -1.0011   0.0005    -1.0005   1.0015];
%fm = [fm1,-fm1(4)-fm1(5)];
%fm = [0 1 1; -1 0 0; 1 0 0];
%centerX = 0;
%centerY = 0;
%diam = 1;
%iplot = 1;
...
if nc == 1       % 1 contour (closed unless single vertical plane)
    ...
    if d2 > 2 % single vertical plane
        ...
        xyPts = {[sE*c1(:,1),sN*c1(:,2)]};
        BW = mpoly2mask(xyPts, xVec, yVec);
        ...
        if uv > 0   % fill inside contour
            ...
            B = false(2);
            B(2,1) = 1;
            xyPts = {[u;w]',[sE*c(:,1),sN*c(:,2)]};
            BW = mpoly2mask(xyPts, xVec, yVec,B);
            BW = (BW - 1 )*-1;
    ...
    if uvc < 0
        ...
        xyPts = {[sE*c1(:,1),sN*c1(:,2)]};
        BW1 = mpoly2mask(xyPts, xVec, yVec);
        BW1 = (BW1 - 1 )*-1;            
    else
        if iplot == 1
            disp('CASE 4')
    ...
    if uvc < 0
        ...
        xyPts = {[sE*c2(:,1),sN*c2(:,2)]};
        BW2 = mpoly2mask(xyPts, xVec, yVec);
        BW2 = (BW2 - 1 )*-1;            
    else
        if iplot == 1
            disp('CASE 6')
