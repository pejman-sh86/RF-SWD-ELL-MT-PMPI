...
cov_mat_file  = strcat(filebase,'_cov.txt');
cov_mat_file2 = strcat(filebase,'_cov.mat');
logfile       = strcat(filebase,'_cov.log');
sdfile        = strcat(filebase,'_sdev.txt');
replicafile   = strcat(filebase,'_rep.dat');
residualfile  = strcat(filebase,'_res.txt');
plotfile1     = strcat(filebase,'_axx.png');
plotfile2     = strcat(filebase,'_ks.png');
plotfile3     = strcat(filebase,'_cum.png');
ibl = 1;

load(data);
NBAND = length(F(1).freq);
freq = F(1).freq;
M = 7;
widths = 4.;
widthe = 60.;
...
%rep = dlmread(replicafile);
%dat = dlmread(datafile);
%res = dat(5:4+NBAND,:)-rep(1:NBAND,:);
[res] = read_r_replica(residualfile,NBAND,M);

load('critical_values.mat');
[sd2] = read_r_replica(sdfile,NBAND,M);
...
%  p = 0;
  ...
  [pass2,value2,bereik2,opp2,dplot2] = kolmogorov(F(ifreq).reshat/std(F(ifreq).reshat),k+1);
  if(value2 < foo(end,3))
