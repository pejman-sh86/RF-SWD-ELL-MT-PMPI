...
c          write(*,*) 'Phase order and iph: ', phase_order(iph), ' ', iph
        end do
...
c -----------------------------------
c Sobroutine to create phases importan for receiver function calculation 
...
c        numph=numph+1
c        write(*,*) 'Current phase number unconverted:', numph
c        nseg(numph)=nlay+2*blay
c        do iseg=1,nlay
c          phaselist(iseg,1,numph)=nlay-iseg+1
c          phaselist(iseg,2,numph)=1
c        end do
c        do iseg=(nlay+1),(nlay+blay)
c          phaselist(iseg,1,numph)=iseg-nlay
c          phaselist(iseg,2,numph)=4
c        end do
c        do iseg=(nlay+blay+1),nseg(numph)
c          phaselist(iseg,1,numph)=nseg(numph)-iseg+1
c          phaselist(iseg,2,numph)=1
c        end do
