function [fig]=ffi_fix_slip_uncertainty_log(filename,isyn,ilin,sl_ens,xx);
...
for iz = 1:NDEP-2;
  for ir = ir1:NRAN-(ir2-1);
    ...
    [aa,bb]=stairs(slout,n1sl,'k');
    patch(aa,bb,[0.8,0.8,0.8]);
    stairs(slout,n1sl,'k');
    axis tight
...
%    if(ilin == 1);plot(xx/scale(iz,ir),lng(:,iz,ir),'-b');end;
