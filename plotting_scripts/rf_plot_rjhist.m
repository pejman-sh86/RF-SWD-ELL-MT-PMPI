...
%        h(idxz)     = h(idxz) + 1;
   ...
   for iz=1:NZ
      ...
      if(NPL == 4);[Na(iz,:),binsa] = hist(a(:,iz),alim);end;
      if(NPL == 5);[Nt(iz,:),binst] = hist(t(:,iz),tlim);end;
   ...
   if(inorm == 0);
     for iz=1:NZ
       ...
       if(NPL == 4);Na(iz,:) = Na(iz,:)/trapz(binsa,Na(iz,:));end;
       if(NPL == 5);Nt(iz,:) = Nt(iz,:)/trapz(binst,Nt(iz,:));end;
     end;
   end;
