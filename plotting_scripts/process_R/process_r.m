...
i_fgauss  = 1;  % Gaussian freq ave (Harrison range ave paper)
i_resamp  = 0;  % Resample to even angle spacing:
...
sd_noise    = 0.05;	% RMS value for trace 133 of Charles' data
		% for SNR 12
...
%% QPE TEST MODELS
%%
%[trial,fileroot,fileroot2,fileext,zs,zr,F,nang_re, ...
%freq,sd,frbw,freq_low,freq_up,cnmo,t0,rmx] = s02_1_300_3150_mind_geo_rj();

%[trial,fileroot,fileroot2,fileext,zs,zr,F,nang_re, ...
%freq,sd,frbw,freq_low,freq_up,cnmo,t0,rmx] = s16_1_500_6000_geo_rj();

%[trial,fileroot,fileroot2,fileext,zs,zr,F,nang_re, ...
%freq,sd,frbw,freq_low,freq_up,cnmo,t0,rmx] = scotia_1_500_6000_geo_rj();

[trial,fileroot,fileroot2,fileext,zs,zr,F,nang_re, ...
freq,sd,frbw,freq_low,freq_up,cnmo,t0,rmx] = s13_1_500_6300_geo_rj();

%[trial,fileroot,fileroot2,fileext,zs,zr,F,nang_re, ...
%freq,sd,frbw,freq_low,freq_up,cnmo,t0,rmx] = s01_1_400_2000_geo_rj();
...
svpfile  = strcat(fileroot2,'_svp.dat');
%svpfile  = 'svp.dat';
...
outfileb = strcat(fileroot2,fileext,'_exstd.txt');
...
for ir = 1:nr	% loop over shots (ranges)
    ...
    status(:,ir),p(:,ir)] = rayfast(paths,z,c,zs,zr,x.r(ir),r_tol);
    
%    if ir == 100%ieven
%        plot([0 x.r(ir)],[zr zs],'--k');
%        plot([x.r(ir)-xd(ir) x.r(ir)],[zr zs],'--k');
%        ieven = ieven +4;
%    end
%    x.r(ir),theta(ir)
...
[figh_red] = reduce_sqrt(x,tsq_lim,5e2,tnmo,tsq2,cnmo,fig);
saveas(figh_red,red_file,'epsc2');
fig = fig+1;

if(trial == 1)
[figh_tr] = raw_plot(x, t_lim, 5e2,tnmo,cnmo,fig);
...
%  Window 1:
    ...
    if(i1(1)+ii-1 <= length(x.t_ax))
        ...
        win1.t(ir,1:length(x.t_ax)-i1(1)+1) = x.t_ax(i1(1):end);
        win1.t(ir,length(x.t_ax)-i1(1)+2:end) = x.t_ax(1:(i1(1)+ii-1)-...
                                          length(x.t_ax))+x.t_ax(end);
    end
...
%  Window 2:
    ...
    if(i2(1)+ii-1 <= length(x.t_ax))
        ...
        win2.t(ir,1:length(x.t_ax)-i2(1)+1) = x.t_ax(i2(1):end);
        win2.t(ir,length(x.t_ax)-i2(1)+2:end) = x.t_ax(1:(i2(1)+ii-1)-...
                                          length(x.t_ax))+x.t_ax(end);
    end
...
%        pr_ave(:,ifr) = sqrt(mean(pr(:,idx).^2,2));
        ...
        qd_ave(:,ifr) = sqrt(mean(qd(:,idx).*conj(qd(:,idx)),2));
        pr_ave(:,ifr) = sqrt(mean(pr(:,idx).*conj(pr(:,idx)),2));

%        sen(ifr) = vsystsen(0,0,x.eq(1),freq(ifr));
...
for iter = 1:niter
    ...
    if i_linint == 0
        ...
        [pol,S,mu] = polyfit(x.r',qd_ave(:,ifr),polygrade);
        [qd_avei(:,ifr),delta(:,ifr)]  = polyval(pol,xd,S,mu);
        [qd_avei2(:,ifr),delta2(:,ifr)] = polyval(pol,x.r',S,mu);

    else

        qd_avei(:,ifr)   = interp1(x.r,qd_ave(:,ifr),xd);
        qd_avei2(:,ifr)  = interp1(xd,qd_avei(:,ifr),x.r);
...
%s04_ang = ang(1,:);
%s04_qd_ave = qd_ave;
%save s04_source.mat s04_ang s04_qd_ave;
%-----------------------------------------------------------------------
%  Calculate R
%-----------------------------------------------------------------------
...
%    theta_even = [theta(1):(theta(end)-theta(1))/(nang_re):theta(end)]+((theta(end)-theta(1))/(nang_re))/2.0;
%    theta_even = theta_even(1:nang_re);
%    theta_even = [theta(1):(45.-theta(1))/nang_re:45.];
...
%
    ...
    theta_even = [theta(1):(theta(end)-theta(1))/(nang_re):theta(end)]...
                 +((theta(end)-theta(1))/(nang_re))/2.0;
    theta_even_lo = [theta(1):(theta(end)-theta(1))/(nang_re):theta(end)];
    theta_even_hi = [theta(1):(theta(end)-theta(1))/(nang_re):theta(end)]...
                    +((theta(end)-theta(1))/(nang_re));
    theta_even = theta_even(1:nang_re);
%    theta_even = [theta(1):(60.-theta(1))/nang_re:60.];
    ...
    for ifr = 1:nf
      for iang = 1:length(theta_even)
        idx = find(Rnan(ifr).theta >= theta_even_lo(iang) & ...
                   Rnan(ifr).theta < theta_even_hi(iang));
        if(idx)
...
if(i_syn == 1)
  for ifr = 1:nf	% Loop over shots
      ...
      noise = sd_noise * randn(length(R(:,ifr)),1);
      R(:,ifr) = R(:,ifr) + noise;
...
%            plot(Rnan(ifr).theta,0.5*ones(size(Rnan(ifr).theta)),'or');
            ...
            set(gca,'XLim',[0 90],'YLim',[0 1.3]);
            set(gca,'XTick',[30 60 90],'XTickLabel',{'30';'60';'90'});
...
xlabel('Range (m)','FontSize',16); ylabel('Reduced Time (s)','FontSize',16);
axis([min(x.r) max(x.r) tsq_lim]); box on;
set(gca,'YDir','reverse','FontSize',16);% title ('Traces');
hold on;
