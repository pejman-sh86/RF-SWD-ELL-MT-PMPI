...
inorm   = 2; %% Normalize profile marginals line by line%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
isyn    = 1; %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
pmax = [];
if iseis == 1
    pmin = [pmin; min(vel_ref(:,2))-dVs];
    pmax = [pmax; max(vel_ref(:,2))+dVs];
    if ivpvs == 1
       pmin = [pmin; min(vel_ref(:,3))-dVpVs];
       pmax = [pmax; max(vel_ref(:,3))+dVpVs];
    end
end
if imt == 1
%     pmin = [pmin; sigmamin];
...
%isyn = 1;
Npanel = 8;%7;
NSD2 = 3; %maximum number of datasets
font_size = 25;
...
ilabel = 1;
if(isd == 1)
   %sdmax = [.5e1, .5e1, .5e1];%[2.e-4, 3.e-2, 7.e-2];%[1.e-3, 1.e-1, 1.e-1];%[4., 2., 1.2];%[0.03, 0.12, 0.03];%
   %sdmin = [1.e0, 1.e-0, 1.e-1];%[5.e-5, 1.e-2, 4.e-2];%[1.e-5, 1.e-3, 1.e-3];%[1.e-0, 1.e-0, 1.e-0];%

   %MT-SWD-RF
   sdlabel = {'MT', 'SWD', 'RF'};
%    sdpartru = [1.e-4,2.e-2,5.e-2];
%    sdmax = [1.7e-4, 4.e-2, 5.8e-2]; %sim8
%    sdmin = [5.e-5, 1.3e-2, 3.8e-2];%sim8
%    XTick = { [1. 1.5]*1.e-4, [ 2. 3.]*1.e-2, [4. 5.]*1e-2 }; %sim8
%    sdmax = [2.e-4, 3.e-2, 7.e-2];%sim7
%    sdmin = [5.e-5, 1.e-2, 4.e-2];%sim7
%    XTick = { [1. 1.5]*1.e-4, [ 2. 3.]*1.e-2, [5. 7.]*1e-2 }; %sim7
%    sdmin = [1.e-0, 1.e-0, 1.e-0]; %sim9
%    sdmax = [5.2, 2.3, 1.4]; %sim9
%    XTick = { [1., 3., 5.], [1., 1.5], [1., 1.2] }; %sim9
   sdmin = [1.e-0, 1.e-0, 1.e-0]; %sim10
   sdmax = [3.6, 2.3, 1.2]; %sim10
   XTick = { [1., 2., 3.], [1., 1.5], [1., 1.1] }; %sim10
% %    sdmin = [1.e-0, 1.e-0, 1.e-0]; %real
% %    sdmax = [5., 2., 1.2]; %real
% %    sdmin = [1.e-5, 1.e-0, 1.e-0]; %real
% %    sdmax = [5.e-4, 2., 1.2]; %real
% %    XTick = { [1., 2., 3.], [1., 1.5], [1., 1.1] }; %real

%    SWD-RF
%    sdlabel = {'SWD', 'RF'};
% %    sdpartru = [2.e-2,5.e-2];
% %    sdmax = [4.e-2, 5.8e-2]; %sim8
% %    sdmin = [1.3e-2, 3.8e-2]; %sim8
% %    XTick = { [ 2. 3.]*1.e-2, [4. 5.]*1e-2 }; %sim8
% %    sdmax = [3.e-2, 7.e-2];%sim7
% %    sdmin = [1.e-2, 4.e-2];%sim7
% %    XTick = { [ 2. 3.]*1.e-2, [5. 7.]*1e-2 }; %sim7
% %    sdmin = [1.e-0, 1.e-0]; %sim9
% %    sdmax = [2.3, 1.3]; %sim9
% %    XTick = {[1., 1.5], [1., 1.2] }; %sim9
%    sdmin = [1.e-0, 1.e-0]; %sim10
%    sdmax = [2.3, 1.2]; %sim10
%    XTick = { [1., 1.5], [1., 1.1] }; %sim10
% %    sdmin = [1.e-0, 1.e-0]; %real
% %    sdmax = [2., 1.2]; %real
% %    XTick = { [1., 1.5], [1., 1.1] }; %real
...
% %    XTick = { [1., 1.1] }; %real
   ...
   Pmax =  .25; %0.35; %sim7:0.1 sim8:.09 sim9: 0.2 sim10: 0.25 real:0.35

%    XTick = { [7.e-5, 1.e-4, 1.5e-4], [7.e-3, 1.e-2, 1.5e-2], [7.e-3, 1.e-2, 1.5e-2] }; %sim
%    exponent = [-4, -2, -2]; %sim
%    XTick = { [1. 1.5]*1.e-4, [ 2. 3.]*1.e-2, [5. 6.]*1e-2 }; %sim6
%    XTick = { [1., 2., 3.], [1., 1.5], [1., 1.1] }; %real
...
% loc(1,5) = loc(1,5) - spw2/4.;
% loc(1,6) = loc(1,6) - 2.*spw2/4.;
% XTick = [5:5:10]; %sim 7, sim8
XTick = [5:10:15]; %sim9, real
Pmax = 0.45; %real: .7 sim8:.6 sim7:.65 sim9:.45 sim10:.8
% Pmax = 1.
NVMX2 = 20;    %sim8: 15 sim7:15 sim9:20 sim10:20
%NVMX2 = NVMX;
...
%%
if(imarg == 1)
   NZ = 400; %400; %4000, 2000
   NZI= 100; %400; %4000, 2000
   nsmooth= ceil(NZ/80.);
   NC = 100; %200; %1000, 400
   NR = 100;
   NA = 100;
...
spw = [spw1 spw2, spw2, spw3, spw2, spw3, spw2, spw2, spw3];
loc = zeros(2,9);
loc(2,:) = .14;
loc(1,1) = .08;
for ii = 2:9
...
% set(h5,'TickDir','in','XTickLabel',[]);
% h6 = subplot('Position',[loc(1,6) loc(2,6) spw2 sph]); hold on; box on;  %%!!!!!!
% set(h6,'YDir','reverse','TickDir','in','YTickLabel',[]);
% set(h6,'TickDir','in','XTickLabel',[]);
% h7 = subplot('Position',[loc(1,7) loc(2,7) spw3 sph]); hold on; box on;  %%!!!!!!
% set(h7,'YDir','reverse','TickDir','in','YTickLabel',[]);
% set(h7,'TickDir','in','XTickLabel',[]);
% h8 = subplot('Position',[loc(1,8) loc(2,8) spw2 sph]); hold on; box on;  %%!!!!!!
% set(h8,'YDir','reverse','TickDir','in','YTickLabel',[]);
% set(h8,'TickDir','in','XTickLabel',[]);
%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%
h5 = subplot('Position',[loc(1,5) loc(2,5) spw2 sph]); hold on; box on;  %%!!!!!!
set(h5,'YDir','reverse','TickDir','in','YTickLabel',[]);
set(h5,'TickDir','in','XTickLabel',[]);
h6 = subplot('Position',[loc(1,6) loc(2,6) spw3 sph]); hold on; box on;  %%!!!!!!
set(h6,'YDir','reverse','TickDir','in','YTickLabel',[]);
set(h6,'TickDir','in','XTickLabel',[]);
h7 = subplot('Position',[loc(1,7) loc(2,7) spw2 sph]); hold on; box on;  %%!!!!!!
set(h7,'YDir','reverse','TickDir','in','YTickLabel',[]);
set(h7,'TickDir','in','XTickLabel',[]);
h8 = subplot('Position',[loc(1,8) loc(2,8) spw2 sph]); hold on; box on;  %%!!!!!!
set(h8,'YDir','reverse','TickDir','in','YTickLabel',[]);
set(h8,'TickDir','in','XTickLabel',[]);
h9 = subplot('Position',[loc(1,9) loc(2,9) spw3 sph]); hold on; box on;  %%!!!!!!
set(h9,'YDir','reverse','TickDir','in','YTickLabel',[]);
set(h9,'TickDir','in','XTickLabel',[]);
%%%%%%%%%%%%%%%
...
if(ivref == 1)
   ...
   h1=plot(vel_ref2(:,2)+dVs,vel_ref2(:,1),'-w','LineWidth',lwp);
   h1=plot(vel_ref2(:,2)+dVs,vel_ref2(:,1),'--k','LineWidth',lwp);
...
if(ivref == 1 && ivpvs == 1)
   ...
   h2=plot(vel_ref2(:,3)+dVpVs,vel_ref2(:,1),'-w','LineWidth',lwp);
   h2=plot(vel_ref2(:,3)+dVpVs,vel_ref2(:,1),'--k','LineWidth',lwp);
...
if(ivref == 0)
  ...
  set(gca,'Fontsize',14,'XLim',[-dVs dVs],'YLim',[0 hmax2]);
  [hdens]=cloudPlot(Vs(:,2),Vs(:,1),[-dVs dVs 0 hmax],true,[1001 1001]);
  set(gca,'XTick',[-dVs:.2:dVs]);
  xlabel('dV_S (m/s)');
end;
...
if(ivref == 0)
  ...
  set(gca,'Fontsize',14,'XLim',[-dVs dVs],'YLim',[0 hmax2]);
  [hdens]=cloudPlot(VpVs(:,2),VpVs(:,1),[-dVpVs dVpVs 0 hmax],true,[1001 1001]);
  set(gca,'XTick',[-dVpVs:2*dVpVs/5:dVpVs]);
  xlabel('dVpVs');
end;
%colormap( 1-hot );
%colormap( 1-gray(256) );
