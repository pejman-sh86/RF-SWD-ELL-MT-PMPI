%================================================================
%   FORWARD.M
%================================================================
function [E] = forward_hol_lays_assa(m,F,frbw);
global nlay nlayfix fix_hs;
%nlay=80
%
% Setting up the environment in the (nlay+2)x4 Array geo_sin:
%

nfreq = length(F(1).freq);
uhs = [NaN F(1).cw 0 F(1).rw];
lhs = [NaN m(end-2) m(end)/(m(end-2)/1000) m(end-1)];
%
% Fixed layers:
%
iipar = [1 2 4 3];
if nlayfix > 0
    idx = 1;
    for ilay = 1:nlayfix
        for ipar = 1:4
            lay(ilay,iipar(ipar)) = F(1).mfix(idx);
            if iipar(ipar) == 3
                lay(ilay,iipar(ipar)) = F(1).mfix(idx)/ ... 
                                        (F(1).mfix(idx-2)/1000);
            end
            idx = idx + 1;
        end
    end

    if fix_hs > 0
        lay(nlayfix+1,:) = [m(1) F(1).mfix(end-2) F(1).mfix(end)/ ...
                           (F(1).mfix(end-2)/1000) F(1).mfix(end-1)];
        laycount = nlayfix + 1;
        idx = 2;
    else
        laycount = nlayfix;
        idx = 1;
    end

    if nlay > 0

        for ilay = laycount+1:laycount+nlay

            for ipar = 1:4

                lay(ilay,iipar(ipar)) = m(idx);

                if iipar(ipar) == 3

                    lay(ilay,3) = m(idx)/(m(idx-2)/1000);

                end

                idx = idx + 1;

            end; end; end; 
else
    if nlay > 0

        idx = 1;

        for ilay = 1:nlay

            for ipar = 1:4

                lay(ilay,iipar(ipar)) = m(idx);

                if iipar(ipar) == 3

                    lay(ilay,3) = m(idx)/(m(idx-2)/1000);

                end

                idx = idx + 1;
         end; end; end;
end;

geo_sin=[uhs; lay; lhs];
%save bla;

%return;
%
% Compute reflectivity:
%
if((m > F(1).minlim) & (m < F(1).maxlim))
  E = 0;
  for i=1:nfreq
    [ref] = ref_nlay_gfav3(F(i).ang,geo_sin,F(1).freq(i),frbw);% compute Reflection
%    ref         = -20*log10(abs(ref));
    ref         = abs(ref);
    Fm(i).dat = reshape(ref,length(ref),1);
    F(i).dat = reshape(F(i).dat,length(F(i).dat),1);
    E = E + length(find(F(1).Rex(:,1))~=0)/2*log(sum(((Fm(i).dat-F(i).dat).*F(1).Rex(:,i)).^2));
%    E = E + sum((Fm(i).dat-F(i).dat).^2)/(2*F(i).sd^2);
    Rtmp(i,:) = ref;
  end
%  E = E/10^3;
else
  disp('Out of bounds')
  for i=1:nfreq
    [ref] = ref_nlay3(F(i).ang,geo_sin,F(1).freq(i));% compute Reflection
%    ref         = -20*log10(abs(ref));
    ref         = abs(ref);
    Fm(i).dat = ref';
  end
  E = 10^50.;
end

%Rtmp(i+1,:) = F(i).ang;
%save('replica.txt','Rtmp','-ascii');
return;
% ---------------------------------------------------------------
% ...this is the end my fiend.
% EOF
