function [] = plot_rjhist_mfi(filename);
...
pmin = [1490  1.3 0.]';
pmax = [1580  2.0 1.0]';
minlimsw = [3.75,  8., 130., 1516., 1510., 1505., 1505.];
maxlimsw = [3.95, 14., 134., 1524., 1518., 1513., 1513.];
thinstep = 10;
...
for j=1:NPT;
   ...
   idx2 = find(A(idx1,end-1)==j);
   [AX,H1,H2] = plotyy([1:length(idx2)],...
                logPPD(idx1(idx2)),[1:length(idx2)],...
                A(idx1(idx2),4));
   set(AX(1),'YTick',[0:40:1000]);
   if(rem(i-1,nx)==0);
      set(get(AX(1),'Ylabel'),'String','logPPD')
      set(AX(1),'YTickLabel',[0:40:1000]);
   else;
      set(AX(1),'YTickLabel',[]);
   end;
...
for i=1:size(sw,2)
   ...
   subplot(h1);hold on;box on;
   [n,lim]=hist(sw(:,i),30);n = [0, n, 0];lim = [lim(1) lim lim(end)];
   dx = mean(diff(lim));
   n = n/(sum(n)*dx);
...
for ik = 1:length(nmod_k);
   plprof(mapk(ik).par,hmax2,col(ik,:),1,0);
end;
set(gca,'YTickLabel',[]);
...
for ik = 1:length(nmod_k);
   plprof(mapk(ik).par,hmax2,col(ik,:),2,0);
end;
set(gca,'YTickLabel',[]);
...
xlabel('Attenuation');
for ik = 1:length(nmod_k);
   plprof(mapk(ik).par,hmax2,col(ik,:),3,0);
end;
set(gca,'YTickLabel',[]);
box on;
...
set(gca,'YTickLabel',[]);
if(pmax(1)<40.);
   set(gca,'XTickLabel',[1480 1500 1520 1540]);
   set(gca,'XTick',[1480 1500 1520 1540]);
else
   set(gca,'XTickLabel',[1480 1500 1520 1540]);
   set(gca,'XTick',[1480 1500 1520 1540]);
end;
...
set(gca,'YTickLabel',[]);
set(gca,'XTickLabel',[1.3 1.6 1.9 2.2]);
set(gca,'XTick',[1.3 1.6 1.9 2.2]);
box on;
