...
for iidata = 1:nset
    for ifile=1:2
        switch ifile
            case 1
                FileName=[FileRoot '_hist1.dat' int2str(iidata)];  % result from sampling 1
                fid  = fopen( FileName,'r');
                A1 = fscanf(fid,'%f',[npar,inf]);
                fclose(fid);
            case 2
                FileName=[FileRoot '_hist2.dat' int2str(iidata)];  % result from sampling 2
                fid  = fopen( FileName,'r');
                A2 = fscanf(fid,'%f',[npar,inf]);
                fclose(fid);
        end;
    end
    switch iidata
