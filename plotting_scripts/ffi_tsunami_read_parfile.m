...
s = fgets(fid); %% IVARPAR:       Selects likelihood function (0=implicit, 1=)
IVARPAR = sscanf(s, '%d'); % convert to number
s = fgets(fid); %% IBD_SINLGE:       Selects likelihood function (0=implicit, 1=)
IBD_SINGLE = sscanf(s, '%d'); % convert to number
s = fgets(fid); %% NVMX:       Max number of nodes.  
NVMX = sscanf(s, '%d'); % convert to number
s = fgets(fid); %% NTW:        Number of parameters per node.
NTW = sscanf(s, '%d'); % convert to number
