...
for i=1:NPV;
  ...
  set(gca,'FontSize',14,'layer','top','LineWidth',1)
  plot(AS(idx,4+NVMX*NPV+NMISC+NSDGPS+i),'k');
...
for iz = 1:NDEP;
  ...
  for ir = 1:NRAN;
    ...
    d  = sqrt(dx.*dx + dz.*dz);
    [dmin,iv] = min(d);
    sl1_evmap(iz,ir) = mapvoro(iv,3);
    sl2_evmap(iz,ir) = mapvoro(iv,4);
    Vr_evmap(iz,ir) = 1./mapvoro(iv,5);
  end;
end;
%%
...
ffi_convert_histfiles(filebase);
load hists.mat
%%
%% Compute 95% HPD map.
%%
...
set(gca,'FontSize',14,'layer','top','LineWidth',1)
imagesc(x_ev,z_ev,sl2_evmap);shading flat;
for i=1:kmap;
  plot(x(i),z(i),'.w','Markersize',7)
  plot(x(i),z(i),'.k','Markersize',5)
end;
