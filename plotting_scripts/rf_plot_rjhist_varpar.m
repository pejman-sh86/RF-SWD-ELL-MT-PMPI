...
ilabel = 1;
if(isd == 1)
   sdmax = [10., 10., 10., 10.];%[0.03, 0.12, 0.03];[1.e-1, 1.e-1, 1.e-3, 1.e-3];
   sdmin = [1.e-0, 1.e-0, 1.e-0, 1.e-0];[1.e-3, 1.e-3, 1.e-5, 1.e-5];
   fig4=figure;
   %nx = NSD/3;
   %ny = 3;
   nx = NSD;
   ny = 1;
   xim = 0.03; %it was 0.01 originally
   yim = 0.06;
   xymarg = [0.1 0.04 0.04 0.14];
   [loc,spw,sph] = get_loc(nx,ny,xim,yim,xymarg);
   ilabel = 1;
   for i=1:NSD;
     ...
     isdmx = i; %%mine
     %set(gca,'YLim',[0.0 sdmax(isdmx)],'YTick',[0.0:0.01:0.1])
     %set(gca,'XLim',[0.0 0.05],'XTick',[0.0:0.01:0.1])
     set(gca,'YLim',[sdmin(isdmx), sdmax(isdmx)]) %,'YTick',[0.0:0.01:0.1])
     set(gca,'XLim',[0.0 0.05])%,'XTick',[0.0:0.01:0.1])
     %if(i > 1);set(gca,'YTickLabel',[]);end;
     box on;
     %text(.01,.0475,sdlabel(ilabel),'FontSize',12,'Color',[0,0,0]);  %mine
     if(isyn == 1);
       plot([0 0.05],[sdpartru(i) sdpartru(i)],'-w');
       plot([0 0.05],[sdpartru(i) sdpartru(i)],'--k');
     end;
...
for ipar=1:NPL-1;
  h1 = subplot('Position',[loc(1,ipar) loc(2,ipar) spw-0.01 sph]); % I have subtracted 0.01
  hold on; box off;
  set(gca,'FontSize',12);
  h2 = subplot('Position',[loc(1,ipar+NPL-1) loc(2,ipar+NPL-1) spw-0.01 sph]);
  hold on; box off;
  set(gca,'FontSize',12);

  subplot(h1);
  set(gca,'FontSize',10); % I have changed the fontsize
  ...
  subplot(h2);
  set(gca,'FontSize',12); % I have changed the fontsize
  [n,lim]=hist(kvpar2(:,ipar),[0:NVMX]);n = [0, n, 0];lim = [lim(1) lim lim(end)];
  n = n/sum(n);
  lim = lim - (lim(3)-lim(2))/2;
  [xx,yy]=stairs(lim,n,'k');
  patch(xx,yy,[0.8,0.8,0.8]);
  ...
  if(ipar==1)
      if(imt==1)
          xlabel('No. nodes with log-resistivity');
      else
          xlabel('No. nodes with V_P');
          xlabel('No. nodes with V_S');
      end
  end
  if(ipar == 2)
      if (imt==1)
          ...
          if (ivpvs==1)
              xlabel('No. nodes with V_P/V_S ratio');
          end
      end
%       if (ivpvs==1)
%           xlabel('No. nodes with V_P/V_S ratio');
%       else
%           xlabel('No. nodes with log-conductivity');
%       end
  end
  if(ipar == 3);xlabel('No. nodes with dip');end;
%   if(ipar == 3);xlabel('No. nodes with log-conductivity');end;
  if(ipar == 3);xlabel('No. nodes with V_P/V_S ratio');end;
...
loc = [0.08,  0.01+0.18, 0.03+0.4533, 0.05+0.7266;...
       0.14, 0.14, 0.14, 0.14]; 
...
h4 = subplot('Position',[loc(1,1) loc(2,1) spw1+0.01 sph]);
hold on; box off;
...
if(imarg == 1)
  ...
  patch(xx,yy,[0.8,0.8,0.8]);
  %[xx,yy]=stairs(h,z,'-k');
  set(gca,'Fontsize',font_size,'YLim',[0 hmax2],'XLim',xxlim);
  ...
  if isyn == 1
      ...
      plot(xxlim, [vorotru(in,1) vorotru(in,1)], '--r')
      end
      box on;
  end
end;
...
set(h1,'layer','top');
set(gca,'Fontsize',font_size,'XLim',[pmin(1) pmax(1)],'YLim',[0 hmax2]);
...
set(h2,'layer','top')
set(gca,'Fontsize',font_size,'XLim',[pmin(2) pmax(2)],'YLim',[0 hmax2]);
set(gca,'YDir','reverse','TickDir','out','YTickLabel',[]);
if ivpvs == 1
...
if(ivref == 1 && ivpvs == 1)
   ...
   h1=plot(vel_ref(:,3)+dVpVs,vel_ref(:,1),'-w','LineWidth',2);
   h1=plot(vel_ref(:,3)+dVpVs,vel_ref(:,1),'--k','LineWidth',2);
...
%surf(clim,z,Nc);shading flat;
set(h3,'layer','top');
set(gca,'Fontsize',font_size,'XLim',[pmin(3) pmax(3)],'YLim',[0 hmax2]);
set(gca,'YDir','reverse','TickDir','out','YTickLabel',[]);
if(isyn == 1)
